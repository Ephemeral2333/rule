...
$.KEY_campus_access = '@wuhuhu.ChinaBroadnet.campus_access'
$.KEY_campus_body = '@wuhuhu.ChinaBroadnet.campus_body'
...
!(async () => {
  ...
  $.log('========== 请求拦截 ==========')
  $.log(`URL: ${url}`)
  $.log(`access: ${access ? access.substring(0, 8) + '...' : '❌ 未获取到'}`)
  $.log(`body: ${body ? body.substring(0, 16) + '...' : '❌ 未获取到'}`)

  if (!access || !body) {
    $.log('❌ 凭证为空，请检查重写规则是否正确配置')
    $.msg($.name, '❌ 凭证获取失败', '未能从请求中取到 access 或 body')
    return
  }
