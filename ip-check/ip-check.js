  ...
  if (provider == 'ipinfo') {
    ...
    try {
      ...
      PROXY_INFO = [
        ...
        getIPTypeLine($.lodash_get(body, 'security.hosting')),
        $.lodash_get(arg, 'ORG') == 1
    ...
    try {
      ...
      const res = await http({
        ...
        url: `http://ip-api.com/json${p}?lang=zh-CN&fields=status,message,country,countryCode,region,regionName,city,isp,org,as,query,hosting`,
        headers: {
          'User-Agent':
            'Mozilla/5.0 (iPhone CPU iPhone OS 13_2_3 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/13.0.3 Mobile/15E148 Safari/604.1 Edg/109.0.0.0',
        },
...
function getIPTypeLine(hosting) {
  if (hosting === true) return ['类型:', 'IDC (机房)'].filter(i => i).join(' ')
  if (hosting === false) return ['类型:', 'ISP (运营商)'].filter(i => i).join(' ')
  return undefined
}
